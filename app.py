...
# ---------------------------------------------------------------------------
# GPS HELPER  — paste this anywhere above _onboarding_page()
# ---------------------------------------------------------------------------

def _reverse_geocode(lat: float, lon: float) -> dict:
    """
    Calls Nominatim and returns a dict with:
        village  — best granular place name (village / town / suburb / county)
        state    — Indian state name mapped to the selectbox options
        raw      — full display_name string for debugging
    Returns empty dict on any failure.
    """
    # Map Nominatim state names → selectbox values
    STATE_MAP = {
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "Pakshi-Handloom-App/1.0"},
            timeout=8,
        )
        ...
        # Best village/cluster name — try from most to least granular
        village = (
            addr.get("village")
            or addr.get("town")
            or addr.get("suburb")
        ...
        raw_state = addr.get("state", "").lower().strip()
        state = STATE_MAP.get(raw_state, "Other")
...
# ---------------------------------------------------------------------------
# ONBOARDING PAGE  — full replacement for _onboarding_page()
# ---------------------------------------------------------------------------
def _onboarding_page() -> None:

    # ---- inline parser (unchanged from original) ----
    def _parse_onboarding_text(text: str) -> dict:
        ...
        weave_map = {
            "टिकट": "ikat", "इकट": "ikat", "बनारसी": "banarasi", "कांचीपुरम": "kanjivaram",
            "जामदानी": "jamdani", "तुस्सर": "tussar", "चंदेरी": "chanderi",
            "महेश्वरी": "maheshwari", "पैठणी": "paithani", "पटोला": "patola",
            "कोटा डोरिया": "kota doria", "संबलपुरी": "sambalpuri",
            "इलकल": "ilkal", "वेंकटागिरी": "venkatagiri", "जरी": "zari", "कसावु": "kasavu"
        }
    ...
    for key, default in [
        ...
        ("reg_phone", ""), ("reg_state", ""), ("last_reg_audio_hash", None),
        ("gps_loading", False),
    ]:
    ...
    if st.session_state["onboard_submitted"]:
        ...
        <div style="background:rgba(34,197,94,0.08);border:1.5px solid #22c55e;
            ...
            <div style="font-size:0.85rem;color:var(--text-primary);line-height:1.7;">
                ...
                <span style="color:var(--accent);font-weight:600;">
                    Confirmation will be sent to {d.get("phone","")} via WhatsApp.
                </span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if _oid:
            st.success(f"Weaver ID: {_oid} — Switch to 'Weaver Dashboard' tab.")
        c1, c2 = st.columns(2)
        if c1.button(get_ui_string("onboard_register_another", lang), use_container_width=True):
            st.session_state["onboard_submitted"] = False
            st.session_state["onboard_data"] = {}
            st.rerun()
        if c2.button(get_ui_string("onboard_go_dashboard", lang), use_container_width=True):
            st.session_state["onboard_submitted"] = False
            st.session_state["onboard_data"] = {}
    ...
    # ── GPS button — uses st.components iframe postMessage approach ───────────
    # Instead of a URL redirect (which reloads the page and loses state),
    # we embed a tiny HTML component that calls the browser Geolocation API
    # and writes the result into a Streamlit text_input via JS postMessage,
    # then reads it back on the Python side.

    st.markdown(f"**{get_ui_string('onboard_gps', lang)}**")

    # The component renders a button; on click it resolves coords via browser
    # API then calls Nominatim itself and posts village+state back as JSON.
    gps_html = """
    <style>
      #gps-btn {
        background: #F43397; color: #fff; border: none; border-radius: 10px;
        padding: 0.6rem 1.2rem; font-size: 0.92rem; font-weight: 700;
        cursor: pointer; font-family: Inter, sans-serif; min-height: 44px;
      }
      #gps-btn:disabled { opacity: 0.55; cursor: wait; }
      #gps-status { margin-top: 6px; font-size: 0.80rem; font-family: Inter, sans-serif; color: #6b6b6b; min-height: 18px; }
    </style>
    <button id="gps-btn" onclick="getLocation()">📍 Get My Location</button>
    ...
    function getLocation() {
      ...
      navigator.geolocation.getCurrentPosition(
        function(pos) {
          ...
          status.textContent = 'Got GPS (' + lat.toFixed(4) + ', ' + lon.toFixed(4) + '). Looking up address…';

          fetch(
            'https://nominatim.openstreetmap.org/reverse?lat=' + lat +
            '&lon=' + lon + '&format=json&zoom=14&addressdetails=1',
            { headers: { 'User-Agent': 'Pakshi-Handloom-App/1.0' } }
          )
          .then(function(r) { return r.json(); })
          .then(function(data) {
            var addr = data.address || {};
            var village = addr.village || addr.town || addr.suburb || addr.county || addr.state_district || '';
            var rawState = (addr.state || '').toLowerCase();
            var stateMap = {
            ...
            var state = stateMap[rawState] || 'Other';
            status.textContent = '✅ Found: ' + village + ', ' + state;
            btn.textContent = '✅ Location Found';

            // Post result up to Streamlit parent
          ...
          .catch(function(e) {
            status.textContent = 'Address lookup failed: ' + e.message;
            btn.disabled = false;
    ...
    # Render the GPS component and capture its postMessage output
    gps_result = st.components.v1.html(gps_html, height=80)

    # st.components.v1.html cannot return values to Python directly.
    # So we use a hidden text_input as the bridge: the JS above posts a message
    # that a second component listens to and writes into the input's DOM value,
    # which Streamlit picks up on the next interaction.
    # Simpler alternative that actually works reliably in Streamlit:
    # expose two text inputs that the user can manually confirm, pre-filled by JS.

    # Bridge: listener component writes village/state into hidden inputs via DOM
    bridge_html = """
    <script>
    window.addEventListener('message', function(e) {
      if (!e.data || e.data.type !== 'pakshi_gps') return;
      // Walk up to find Streamlit's shadow inputs and set their values
      function setInput(label, value) {
        ...
        for (var i = 0; i < inputs.length; i++) {
          ...
          if (lbl && lbl.querySelector('label') &&
              lbl.querySelector('label').textContent.trim().toLowerCase().indexOf(label) !== -1) {
      ...
      setInput('village', e.data.village);
      // For state we store in sessionStorage so Python can read it via a query param trick
      window.parent.sessionStorage.setItem('pakshi_gps_village', e.data.village);
      window.parent.sessionStorage.setItem('pakshi_gps_state', e.data.state);
      // Also update the URL so Streamlit's query_params picks it up
      var url = new URL(window.parent.location.href);
      url.searchParams.set('gps_village', e.data.village);
    ...
    # Read GPS result from query params (set by the JS above without a full reload)
    gps_village = st.query_params.get("gps_village", "")
    gps_state   = st.query_params.get("gps_state", "")

    # If we got new GPS data, store it in session_state and clear the params
    if gps_village and gps_village != st.session_state.get("reg_cluster", ""):
        st.session_state["reg_cluster"] = gps_village
        if gps_state:
            st.session_state["reg_state"] = gps_state
        try:
            if "gps_village" in st.query_params:
                del st.query_params["gps_village"]
            if "gps_state" in st.query_params:
                del st.query_params["gps_state"]
        except Exception:
            pass
        st.success(f"📍 Location filled: {gps_village}, {gps_state}")

    # ── Voice input ───────────────────────────────────────────────────────────
    st.markdown(f"""
    <div style="background:rgba(244,51,151,0.07);border:2px solid rgba(244,51,151,0.35);
        border-radius:14px;padding:1rem 1.2rem;margin-bottom:1rem;margin-top:0.8rem;">
        ...
        <div style="font-size:0.82rem;color:var(--text-muted);line-height:1.6;">
            <strong>Say your name, village, weave, and phone number.</strong><br>
    ...
    # ── Form ──────────────────────────────────────────────────────────────────
    # Build state selectbox index from GPS or session_state
    STATE_OPTIONS = [
    ...
    default_state_val = st.session_state.get("reg_state", "")
    state_index = STATE_OPTIONS.index(default_state_val) if default_state_val in STATE_OPTIONS else 0
    ...
    with st.form("onboard_form"):
        ...
        c3, c4 = st.columns(2)
        # Village/cluster — pre-filled by GPS or voice
        cluster = c3.text_input(
            get_ui_string("onboard_cluster", lang),
            value=st.session_state.get("reg_cluster", ""),
            placeholder="e.g. Pochampally",
        )
        # State — pre-selected by GPS
        state = c4.selectbox(get_ui_string("onboard_state", lang), STATE_OPTIONS, index=state_index)
        ...
        if submitted:
            ...
            if not cluster.strip():
                errors.append("Village / Cluster is required.")
            if not fabric:
                errors.append("Select at least one fabric speciality.")
            if not consent:
                errors.append("You must agree to list on Meesho.")
            ...
            if errors:
                ...
                st.session_state["onboard_data"] = {
                    ...
                    "cluster": cluster.strip(), "state": state, "fabric": ", ".join(fabric),
                    "weave": weave.strip(), "min_price": min_p, "delivery_days": delivery,
                    "whatsapp": whatsapp, "accepts_custom": custom, "language": lang_pref,
                }
                st.session_state["onboard_submitted"] = True
                # Clear GPS session state so next registration starts fresh
                for k in ("reg_name","reg_cluster","reg_specialty","reg_phone","reg_state","gps_loading"):
